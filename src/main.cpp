...
int main()
{
    ...
    // 计算运动学逆解
    kinematics.KinematicsInverse(target_linear_speed, target_angular_speed,
    ...
    std::cout << "目标线速度: " << target_linear_speed << " m/s"
              << " 目标角速度: " << target_angular_speed << " rad/s" << std::endl;
    std::cout << "计算得到的左轮速度：" << kinematics.GetMotorSpeed(0) << " rpm"
              << std::endl;
    std::cout << "计算得到的右轮速度：" << kinematics.GetMotorSpeed(1) << " rpm"
              << std::endl;
